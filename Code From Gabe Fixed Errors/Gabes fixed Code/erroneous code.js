...
function checkNumberNegativeOddEven(num) { // GABE CHANGE # 1 - Changed the function name to match the function call
    if (num > 0) {
...
for (let student in studentGrades) {
  console.log(student + ": " + studentGrades[student]) // GABE CHANGE # 2 - Changed student -> studentGrades[student], because student is the key in the dictionary studentGrades
...
function highestNum(n) {
  ...
  for (let i = 1; i < n.length; i ++) {
      if (n[i] > high) { // GABE CHANGE # 5 - This was the last one i found, i didn't see it until after i looked at the 2 js errors.
                        //  I changed n[0] -> n[i] because instead of comparing the zero'th index to the current highest number, we want
                       //   to compare the current index to the current highest number.
          high = n[i];
      }
  }
  return high;
...
console.log("the highest num is " + highestNum(numarray)); 
